...
class Results(SimpleClass):
    ...
    """
    def __init__(
        self, orig_img, path, names, person_states=None, boxes=None, masks=None, probs=None, keypoints=None, obb=None, speed=None, embeds=None
    ) -> None:
        """
        ...
        self.names = names
        self.person_states = None if not person_states else person_states #$#添加person_states参数
    ...
    def plot(
        ...
        if pred_boxes is not None and show_boxes:
            for i, d in enumerate(reversed(pred_boxes)):
                ...
                #￥# 检查是否为JDE任务且有状态信息self.person_states
                if self.person_states and id is not None: 
                    person_states = self.person_states.get(id, f"state:{id}")  # 使用状态标签而不是ID
                    name = names[c] + f" {person_states} "  
                else:  # 原有的ID显示逻辑
...
class Boxes(BaseTensor):
    ...
    def id(self):
        ...
        Notes:
            ...
            For JDE with state predictions, returns state IDs.
