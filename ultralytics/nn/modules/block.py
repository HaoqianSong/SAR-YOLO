...
from .conv import Conv, DSConv, DWConv, GhostConv, LightConv, RepConv, autopad, CBAM
...
class C3k2_CBAM(C3k2):
    """
    C3k2 module with CBAM attention mechanism.
    """
    
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True, kernel_size=7):
        """Initializes C3k2_CBAM module with CBAM attention."""
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        # 添加 CBAM 模块，应用在输出通道 c2 上
        self.cbam = CBAM(c2, kernel_size=kernel_size)
    
    def forward(self, x):
        """Forward pass through C3k2 with CBAM applied to output."""
        # 调用父类 C3k2 的 forward 方法（实际是 C2f 的 forward）
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
    ...
    def forward_split(self, x):
        """Forward pass using split() instead of chunk() - for ONNX export."""
        # 使用 split 而不是 chunk，以兼容 ONNX 导出
        y = self.cv1(x).split((self.c, self.c), 1)
        y = [y[0], y[1]]
        y.extend(m(y[-1]) for m in self.m)
        output = self.cv2(torch.cat(y, 1))
        # 在输出前应用 CBAM（与 forward 保持一致）
        return self.cbam(output)
...
class DSC3k2_CBAM(DSC3k2):
    """
    DSC3k2 module with CBAM attention mechanism.
    """
    ...
    def __init__(
        ...
        """Initializes DSC3k2_CBAM module with CBAM attention."""
        super().__init__(c1, c2, n, dsc3k, e, g, shortcut, k1, k2, d2)
        # 添加 CBAM 模块，应用在输出通道 c2 上
        self.cbam = CBAM(c2, kernel_size=kernel_size)
    
    def forward(self, x):
        """Forward pass through DSC3k2 with CBAM applied to output."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        output = self.cv2(torch.cat(y, 1))
        # 在输出前应用 CBAM
...
class FuseModule(nn.Module):
    ...
    def __init__(self, c_in, channel_adjust):
        ...
        self.channel_adjust = channel_adjust
         # 将 x_cat 的实际通道数转换到 conv_out 期望的通道数
        self.conv_adjusts = nn.ModuleDict()
    ...
    def forward(self, x):
        if len(x) == 3:
            ...
            x0_ds2 = self.downsample(self.downsample(x[0]))  # 第一个输入下采样两次
            x1_ds = self.downsample(x[1])  # 第二个输入上采样
            #x2 = x[2]  # 第三个输入保持不变（作为参考尺寸）
            x3_up = self.upsample(x[3]) # 第四个输入上采样
            x_cat = torch.cat([x0_ds2, x1_ds, x[2], x3_up], dim=1)
            actual_channels = x_cat.shape[1]
            key = str(actual_channels)
             # 获取 x_cat 的实际通道数
            if key not in self.conv_adjusts:
                self.conv_adjusts[key] = Conv(actual_channels, self.c_in, 1)
            #self.conv_adjusts = Conv(x_cat.shape[1], self.c_in, 1)
            out = self.conv_adjusts[key](x_cat)
...
class FullPAD_Tunnel(nn.Module):
    ...
    def forward(self, x):
        if x[0].shape[1] != x[1].shape[1]:
            # 打印模块信息
            module_info = f"模块索引: {getattr(self, 'i', 'unknown')}, 来源: {getattr(self, 'f', 'unknown')}, 类型: {getattr(self, 'type', 'unknown')}"
            print(f"FullPAD_Tunnel通道数不匹配!")
            print(f" ￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥ {module_info}")
            print(f"  x[0].shape={x[0].shape} (通道数={x[0].shape[1]})")
            print(f"  x[1].shape={x[1].shape} (通道数={x[1].shape[1]})")
