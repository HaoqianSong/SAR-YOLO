...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "JDE"
...
class JDE(Detect):
    ...
    def __init__(self, nc=80, embed_dim=128, state_classes=None, ch=()): 
        ...
        if state_classes is not None: #$#embeddings预测状态
            ...
            self.state_predictor = nn.Sequential(nn.Linear(embed_dim, embed_dim // 2), nn.ReLU(), 
                                                    nn.Dropout(0.1), nn.Linear(embed_dim // 2, state_classes)) #$#embeddings预测状态
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            if self.state_classes is not None: #$#embeddings预测状态
                embed_out = self.cv4[i](x[i]) #$#embeddings预测状态
                b, c, h, w = embed_out.shape #$#embeddings预测状态
                embed_flat = embed_out.view(b, c, -1).permute(0, 2, 1)  # (B, H*W, embed_dim) #$#embeddings预测状态
                state_out = self.state_predictor(embed_flat)  # (B, H*W, state_classes) # 通过状态预测网络 #$#embeddings预测状态
                state_out = state_out.permute(0, 2, 1).view(b, self.state_classes, h, w)  # 恢复空间维度 #$#embeddings预测状态
                x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i]), embed_out, state_out), 1) #$#embeddings预测状态
            else:
                x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i]), self.cv4[i](x[i])), 1) #$#embeddings预测状态 
    ...
    def _inference(self, x):
        ...
        if self.export and self.format in {"tflite", "edgetpu"}:
            ...
            dbox = self.decode_bboxes(self.dfl(box), self.anchors.unsqueeze(0)) * self.strides
        if self.state_classes is not None: #$#embeddings预测状态
            return torch.cat((dbox, cls.sigmoid(), emb, state.sigmoid()), 1) #$#embeddings预测状态 添加state.sigmoid()
        else:
