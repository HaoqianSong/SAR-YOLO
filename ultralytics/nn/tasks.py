...
class JDEModel(DetectionModel):
    """YOLOv13 Joint Detection and Embedding (JDE) model."""

    def __init__(self, cfg="yolov13n-jde.yaml", ch=3, nc=None, verbose=True):
        """Initialize YOLOv13 JDE model with given config and parameters."""
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)

    def init_criterion(self):
        """Initialize the loss criterion for the JDEModel."""
        return v13JDELoss(self)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                n = 1
            if m in {C3k2, DSC3k2, C3k2_CBAM, DSC3k2_CBAM}:  # for P/U sizes
                legacy = False
                if scale in "lx":
                    args[3] = True
            if m is A2C2f: 
            ...
            if m in {Detect, Segment, Pose, OBB, JDE}:  #$#
                m.legacy = legacy
