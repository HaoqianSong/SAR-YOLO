...
def verify_image_label(args):
    ...
    im_file, lb_file, prefix, keypoint, num_cls, nkpt, ndim, use_tag= args  #￥# 添加use_tag,
    
    # Number (missing, found, empty, corrupt), message, segments, keypoints
    nm, nf, ne, nc, msg, segments, keypoints, tags = 0, 0, 0, 0, "", [], None, None  # 添加tags
    try:
        # Verify images
        im = Image.open(im_file)
        im.verify()  # PIL verify
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                ...
                elif use_tag:  ##￥# JDE任务支持6列
                    if lb.shape[1] == 6:  #￥#
                        tags = lb[:, 5].astype(np.int32)  ##￥# 提取person_id
                        points = lb[:, 1:5]  ##￥# 只使用前5列做坐标验证
                    else:  #￥#
                        tags = None   #￥#
                        points = lb[:, 1:]   #￥#
                else:
        ...
        lb = lb[:, :5]
        return im_file, lb, shape, segments, keypoints, nm, nf, ne, nc, msg, tags  ##￥# tags,添加tags返回值
    except Exception as e:
        nc = 1
        msg = f"{prefix}WARNING ⚠️ {im_file}: ignoring corrupt image/label: {e}"
        return [None, None, None, None, None, nm, nf, ne, nc, msg, None]  ##￥# 添加None for tags
...
def check_det_dataset(dataset, autodownload=True):
    ...
    # 添加人员ID到名字的映射处理 #$#
    if "person_states" in data:   #$#
        data["person_states"] = check_class_names(data["person_states"]) #$#
        LOGGER.info(f"Loaded {len(data['person_states'])} person_states mappings") #$#
