...
class Mosaic(BaseMixTransform):
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        cls = []
        instances = []
        # 只有在数据集需要tags时才创建tags列表
        use_tags = self.dataset.use_tags #getattr(self.dataset, 'use_tags', False)   #￥# 后来添加这一行
        tags = [] if use_tags else None #￥# 后来添加这一行,从YOLO11-JDE中获取,用于增加"tags"标签信息
        imgsz = self.imgsz * 2  # mosaic imgsz
        for labels in mosaic_labels:
            cls.append(labels["cls"])
            instances.append(labels["instances"])
            if "tags" in labels and use_tags:  #￥# 后来添加这一行,从YOLO11-JDE中获取,用于增加"tags"标签信息
                tags.append(labels["tags"]) #￥# 后来添加这一行,从YOLO11-JDE中获取,用于增加"tags"标签信息
        # Final labels
        final_labels = {
        ...
        if use_tags and tags: #￥# 后来添加这一行,
            final_labels["tags"] = np.concatenate(tags, 0) if len(tags) else None #￥# 后来添加这一行,从YOLO11-JDE中获取,用于增加"tags"标签信息
        final_labels["instances"].clip(imgsz, imgsz)
        good = final_labels["instances"].remove_zero_area_boxes()
        final_labels["cls"] = final_labels["cls"][good]
        if use_tags and "tags" in final_labels and final_labels["tags"] is not None: #￥# 后来添加这一行,
            if len(tags):
                final_labels["tags"] = final_labels["tags"][good]  #￥# 后来添加这一行,从YOLO11-JDE中获取,用于增加"tags"标签信息
            else:
                final_labels["tags"] = None  #￥# 后来添加这一行,从YOLO11-JDE中获取,用于增加"tags"标签信息
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        if "tags" in labels: #￥# 添加这一行,从YOLO11-JDE中获取,用于增加"tags"标签信息
            labels["tags"] = np.concatenate([labels["tags"], labels2["tags"]], 0) #￥# 添加这一行,从YOLO11-JDE中获取,用于增加"tags"标签信息
        return labels
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        img = labels["img"]
        cls = labels["cls"]
        tags = labels.get("tags", None)  #￥# 添加这一行,从YOLO11-JDE中获取,用于增加"tags"标签信息
        ...
        labels["cls"] = cls[i]
        labels["tags"] = tags[i] if tags is not None else None   #$# 添加这一行,从YOLO11-JDE中获取,用于增加"tags"标签信息
        labels["img"] = img
...
class Format:
    ...
    def __call__(self, labels):
        ...
        h, w = img.shape[:2]
        cls = labels.pop("cls")
        tags = labels.pop("tags", None)  #$# 添加这一行  从YOLO11-JDE中获取，用于处理"tags"数组维度不一致问题
        instances = labels.pop("instances")
        ...
        if tags is not None:  #$# 添加这一行  从YOLO11-JDE中获取
            labels["tags"] = torch.from_numpy(tags) if nl else torch.zeros(nl)  #$# 添加这一行  从YOLO11-JDE中获取，将数组转换为张量
