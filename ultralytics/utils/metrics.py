...
class ReIDMetrics(SimpleClass):
    ...
    def get_metrics(self):
        ...
        # Check if we have any embeddings to process #$# 添加这一行  从YOLO11-JDE中获取，
        if not self.embeds or all(len(embed) == 0 for embed in self.embeds): #$# 添加这一行  从YOLO11-JDE中获取，
            # Return default metrics when no embeddings are available #$# 添加这一行  从YOLO11-JDE中获取，
            return { #$# 添加这一行  从YOLO11-JDE中获取，
                "val/pos_cos": 0.0, #$#正样本余弦距离（同一人的embeddings距离） 添加这一行  从YOLO11-JDE中获取，
                "val/neg_cos": 1.0, #$#负样本余弦距离（不同人的embeddings距离） 添加这一行  从YOLO11-JDE中获取，
                "val/pos_euc": 0.0, #$#正样本欧氏距离（同一人的embeddings距离） 添加这一行  从YOLO11-JDE中获取，
                "val/neg_euc": 1.0, #$#负样本欧氏距离（不同人的embeddings距离） 添加这一行  从YOLO11-JDE中获取，
                "val/cos_sep_ratio": 1.0, #$#余弦距离分离比（负样本余弦距离/正样本余弦距离,越大越好） 添加这一行  从YOLO11-JDE中获取，
                "val/euc_sep_ratio": 1.0, #$#欧氏距离分离比（负样本欧氏距离/正样本欧氏距离,越大越好） 添加这一行  从YOLO11-JDE中获取，
                "val/cos_silhouette": 0.0, #$#余弦距离轮廓系数 添加这一行  从YOLO11-JDE中获取，
                "val/euc_silhouette": 0.0, #$#欧氏距离轮廓系数 添加这一行  从YOLO11-JDE中获取，
                "val/davies_bouldin": 0.0, #$#戴维斯-博尔丁(Davies-Bouldin)指数（越小越好） 添加这一行  从YOLO11-JDE中获取，
                "val/calinski_harabasz": 0.0, #$#卡林斯基-哈拉巴斯(Calinski-Harabasz)指数（越大越好） 添加这一行  从YOLO11-JDE中获取，
                "val/r1_acc": 0.0, #$#Rank-1准确率（ReID最重要指标）  添加这一行  从YOLO11-JDE中获取，
                "val/r5_acc": 0.0, #$#Rank-5准确率（ReID重要指标）  添加这一行  从YOLO11-JDE中获取，
                "val/mean_ap": 0.0, #$#平均准确率（ReID重要指标）  添加这一行  从YOLO11-JDE中获取， 
                "val/hota": self.hota, #$#HOTA（High Order Tracking Accuracy）  添加这一行  从YOLO11-JDE中获取，
                "val/mota": self.mota, #$#MOTA（Multiple Object Tracking Accuracy）  添加这一行  从YOLO11-JDE中获取，
                "val/idf1": self.idf1, #$#IDF1（Identity F1）  添加这一行  从YOLO11-JDE中获取，
            } #$# 添加这一行  从YOLO11-JDE中获取，
        ...
        #￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离 检查标签的唯一性，如果只有一个标签直接返回默认值
        unique_tags = np.unique(tags)
        if len(unique_tags) < 2:
            print(f"⚠️  警告：标签数量不足({len(unique_tags)})，embedding数量: {len(embeds)}，返回默认metrics")
            return {
                "val/pos_cos": 0.0,
                "val/neg_cos": 1.0, 
                "val/pos_euc": 0.0,
                ...
                "val/mota": self.mota,
                "val/idf1": self.idf1,
            }#￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离
       
        max_embeddings = 10000  #￥#添加这一行  从YOLO11-JDE中获取 限制最大embedding数量
        if len(embeds) > max_embeddings: #￥#添加这一行  从YOLO11-JDE中获取 限制最大embedding数量
            print(f"⚠️  警告：embedding数量({len(embeds)})超过限制，随机采样{max_embeddings}个") #￥#添加这一行  从YOLO11-JDE中获取 限制最大embedding数量
            #indices = np.random.choice(len(embeds), max_embeddings, replace=False) #￥#添加这一行  从YOLO11-JDE中获取 限制最大embedding数量

            ##￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离 分层采样，确保每个标签都有代表# 计算每个标签应该采样的数量
            samples_per_tag = max(1, max_embeddings // len(unique_tags))
            selected_indices = []
            for tag in unique_tags:
                tag_indices = np.where(tags == tag)[0]
                # 每个标签至少采样1个，最多采样计算出的数量
                n_samples = min(len(tag_indices), samples_per_tag)
                if len(tag_indices) > n_samples:
                    sampled_indices = np.random.choice(tag_indices, n_samples, replace=False)
                else:
                    sampled_indices = tag_indices
                selected_indices.extend(sampled_indices)
            # 如果总数还不够，从剩余的中随机补充
            if len(selected_indices) < max_embeddings:
                remaining_indices = np.setdiff1d(np.arange(len(embeds)), selected_indices)
                if len(remaining_indices) > 0:
                    additional_needed = min(max_embeddings - len(selected_indices), len(remaining_indices))
                    additional_indices = np.random.choice(remaining_indices, additional_needed, replace=False)
                    selected_indices.extend(additional_indices)#￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离
            
            indices = np.array(selected_indices)
            embeds = embeds[indices] #￥#添加这一行  从YOLO11-JDE中获取 限制最大embedding数量
            tags = tags[indices] #￥#添加这一行  从YOLO11-JDE中获取 限制最大embedding数量

        #￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离 再次检查采样后的标签唯一性
            unique_tags_after = np.unique(tags)
            if len(unique_tags_after) < 2:
                print(f"⚠️  警告：采样后标签数量不足({len(unique_tags_after)})，返回默认metrics")
                return {
                    "val/pos_cos": 0.0,
                    "val/neg_cos": 1.0, 
                    "val/pos_euc": 0.0,
        ...
        #print(f"🔍 计算ReID metrics，embedding数量: {len(embeds)}") #￥#添加这一行  从YOLO11-JDE中获取 限制最大embedding数量
        print(f"🔍 计算ReID metrics，embedding数量: {len(embeds)}，唯一标签数: {len(np.unique(tags))}") #￥#添加这一行  从YOLO11-JDE中获取 限制最大embedding数量
        ...
        cos_separation_ratio = neg_cos / pos_cos if pos_cos > 0 else 1.0 #￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离
        euc_separation_ratio = neg_euc / pos_euc if pos_euc > 0 else 1.0 #￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离
        #snr_separation_ratio = neg_snr / pos_snr

        # Compute Silhouette, Davies Bouldin and Calinski Harabasz scores
        ##￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离 这些指标都需要至少2个不同的标签
        try:
            cos_silhouette_score = skm.silhouette_score(cos_distmat, tags, metric='precomputed')
        except ValueError as e:
            print(f"⚠️  余弦silhouette_score计算失败: {e}")
            cos_silhouette_score = 0.0
            
        try:
            euc_silhouette_score = skm.silhouette_score(euc_distmat, tags, metric='precomputed')
        except ValueError as e:
            print(f"⚠️  欧氏silhouette_score计算失败: {e}")
            euc_silhouette_score = 0.0
            
        try:
            davies_bouldin_score = skm.davies_bouldin_score(embeds, tags)
        except ValueError as e:
            print(f"⚠️  davies_bouldin_score计算失败: {e}")
            davies_bouldin_score = 0.0
            
        try:
            calinski_harabasz_score = skm.calinski_harabasz_score(embeds, tags)
        except ValueError as e:
            print(f"⚠️  calinski_harabasz_score计算失败: {e}")
            calinski_harabasz_score = 0.0 #￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离

        metrics = {
            "val/pos_cos": pos_cos if not np.isnan(pos_cos) else 0.0, #￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离
            "val/neg_cos": neg_cos if not np.isnan(neg_cos) else 1.0, #￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离
            "val/pos_euc": pos_euc if not np.isnan(pos_euc) else 0.0, #￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离
            "val/neg_euc": neg_euc if not np.isnan(neg_euc) else 1.0, #￥#检查人员id只有一个时，相同的id没办法做聚类或不同类远离
            #"val/pos_snr": pos_snr,
            #"val/neg_snr": neg_snr,
    ...
    def compute_distmat(embeddings, labels, distance="cosine"):
        ...
        n = len(embeddings)
        chunk_size = 500  # 每次处理1000个embedding

        if n <= chunk_size:  #$# 小数据集直接计算
            ...
            print(f"🔄 分块计算距离矩阵: {n}个embedding，分成{(n-1)//chunk_size + 1}块") #￥#添加这一行  限制最大embedding数量
            distmat = np.zeros((n, n), dtype=np.float32)  #￥#添加这一行  限制最大embedding数量
            for i in range(0, n, chunk_size): #￥#添加这一行  限制最大embedding数量
                for j in range(0, n, chunk_size): #￥#添加这一行  限制最大embedding数量
                    end_i = min(i + chunk_size, n) #￥#添加这一行  限制最大embedding数量
                    end_j = min(j + chunk_size, n) #￥#添加这一行  限制最大embedding数量
                    
                    if distance == "cosine": #￥#添加这一行  限制最大embedding数量
                        chunk_dist = skm.pairwise_distances( #￥#添加这一行  限制最大embedding数量
                            embeddings[i:end_i], embeddings[j:end_j], metric="cosine" #￥#添加这一行  限制最大embedding数量
                        ) #￥#添加这一行  限制最大embedding数量
                    elif distance == "euclidean": #￥#添加这一行  限制最大embedding数量
                        chunk_dist = skm.pairwise_distances( #￥#添加这一行  限制最大embedding数量
                            embeddings[i:end_i], embeddings[j:end_j], metric="euclidean" #￥#添加这一行  限制最大embedding数量
                        ) #￥#添加这一行  限制最大embedding数量
                    elif distance == "snr": #￥#添加这一行  限制最大embedding数量
                        chunk_dist = snr_distance(embeddings[i:end_i], embeddings[j:end_j]) #￥#添加这一行  限制最大embedding数量
                    else: #￥#添加这一行  限制最大embedding数量
                        raise ValueError(f"Invalid distance metric: {distance}") #￥#添加这一行  限制最大embedding数量
                    distmat[i:end_i, j:end_j] = chunk_dist #￥#添加这一行  限制最大embedding数量
